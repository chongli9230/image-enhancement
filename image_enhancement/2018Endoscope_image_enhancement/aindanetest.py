...
def main():
    ...
    #img_path2 = "F:/JZYY/code/2018Endoscope_image_enhancement/result/97/"
    #img_name2 = "ain97.tif"

    aindane = Aindane(img_path + img_name)
    temp = aindane.aindane() 
    #plt.imshow(cv2.cvtColor(np.hstack([self.img_bgr, temp]), cv2.COLOR_BGR2RGB))    
    plt.imshow(cv2.cvtColor(temp, cv2.COLOR_BGR2RGB))
    plt.axis('off')    
    #plt.savefig("./result/aindane" + img_name)
    plt.show()
